 ...
 */
BubbleFish::BubbleFish(Aquarium *aquarium) : Item(aquarium)
{
    mFishImage = make_unique<wxImage>(FishBubbleFish, wxBITMAP_TYPE_ANY);
 ...
 * @return true if hit.
 */
bool BubbleFish::HitTest(int x, int y)
{
    double wid = mFishBitmap->GetWidth();
    double hit = mFishBitmap->GetHeight();
